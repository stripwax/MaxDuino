...
This feature is introduced in v4.00 onwards, and is initially enabled only for a subset of devices (see later)
...
Support for recording requires dedicated external components, not least an additional audio input (e.g. jack socket) and probably some kind of amplifier and/or voltage divider circuitry.  Also, of course, support for recording also requires an additional unused analog input pin on the MCU board.

Currently the firmware support for recording functionality is only implemented for the followiong boards:

* ATMEGA4808 (Thinary Nano Every)
* ATMEGA4809 (Arduino Nano Every)
* SAMD21 (SEEED XIAO M0)

Firmware support for recording using other boards is likely to be added in the future - but only for those
