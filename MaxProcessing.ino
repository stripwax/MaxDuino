...
void TZXProcess()
{
  // calling code will keep calling TZXProcess until currentPeriod != 0
  // to allow for multiple statemachine transitions to occur before we determine
  // what pulse to emit next
  currentPeriod = 0;
  ...
  #ifdef DEBUG
    default:
      // unknown state, shouldn't get here unless we missed a clause above
      HeaderFail();
      break;
  #endif
  }
...
void TZXProcessID()
{
  // TZX ID Processing - this is the big one
  switch(currentID) {
    case ID10:
    ...
    case TAP:
      //Pure Tap file block
      // arguably should be implemented as TASK::TAPFILE , rather than a fake TZX block type
      // but does share code/state (calls StandardBlock) so it's not so bad
      switch(currentBlockTask) {
        case BLOCKTASK::READPARAM:
        ...
        case BLOCKTASK::PAUSE:
          // for TAP, we pause and go straight to the next block READPARAM (there's no block ID)
          currentPeriod = pauseLength;
          bitSet(currentPeriod, 15);
          currentBlockTask = BLOCKTASK::READPARAM;
          if(EndOfFile) currentTask = TASK::REACHED_EOF;
          break;
    ...
    case ZXP:
      // Should be reimplemented as TASK::ZXPFILE , since shares no code with TZX blocktypes
      switch(currentBlockTask) {
        case BLOCKTASK::READPARAM:
          currentChar=0;
          // fallthrough->
    ...
    case ZXO:
      // Should be reimplemented as TASK::ZXOFILE , since shares no code with TZX blocktypes
      switch(currentBlockTask) {
        case BLOCKTASK::READPARAM:
          currentChar=0;
          // fallthrough ->
    ...
    case AYO:                           //AY File - Pure AY file block - no header, must emulate it
      // arguably should be implemented as TASK::AY_FILE , rather than a fake TZX block type
      // but does share code/state (calls StandardBlock) so it's not so bad
      switch(currentBlockTask) {
        case BLOCKTASK::READPARAM:
        ...
        case BLOCKTASK::TDATA:
          if(AYPASS == 1) {
            // header section
            hdrptr = 0;
            currentBlockTask = BLOCKTASK::AY_WRITE_HEADER;
            AYPASS = 2;
          } else {
            // data section
            currentByte = 0xFF;
            currentBit = 8;
            bitChecksum ^= currentByte;
            currentBlockTask = BLOCKTASK::AY_WRITE_DATA_FLAG;
          }
          break;

        case BLOCKTASK::AY_WRITE_HEADER:
          // write TAP Header data from String Vector
          writeHeader2();
          // if header has finished, then writeHeader2 will automatically switch to BLOCKTASK::PAUSE
          break;

        case BLOCKTASK::PAUSE:
          // after write header, we end up here (the first pause, between the header and data)
          currentPeriod = pauseLength;
          bitSet(currentPeriod, 15);

          // get ready to send the data block.
          bitChecksum = 0;  
          bytesRead = 0;
        ...
        case BLOCKTASK::AY_WRITE_DATA_FLAG:
          writeData();
          if (currentBit == 0)
          {
            // finished writing these bits. Now write data from the contents of the AY file
            currentBlockTask = BLOCKTASK::AY_WRITE_DATA;
          }
          break;

        case BLOCKTASK::AY_WRITE_DATA:
          // Data Playback
          if (getData())
          {
            // getData returns True if it got a new byte this time
            bitChecksum ^= currentByte;
          }

          writeData(); 
          // getData changes to BLOCKTASK::PAUSE after reading the last byte
          // we want to intercept that to write our final checksum instead
          if (currentBlockTask == BLOCKTASK::PAUSE)
          {
            currentByte = bitChecksum;
            currentBit = 8;
            currentBlockTask = BLOCKTASK::AY_WRITE_CHECKSUM;
          }
          break;

        case BLOCKTASK::AY_WRITE_CHECKSUM:
          writeData();
          if (currentBit==0)
          {
            EndOfFile=true;
            currentTask = TASK::REACHED_EOF;
          }
          break;
  ...
  #ifdef tapORIC
    case ORIC:
      // Should be reimplemented as TASK::ORIC_FILE , since shares no code with TZX blocktypes
      switch(currentBlockTask) {            
...
void DoNewBlockUpdates()
{
  // Some common things we do every time we start a new block.
  // (although not every block type, just the ones that call this fn)
    ...
    EEPROM_put(BLOCK_EEPROM_START+4+5*block, currentID);
    ...
    #ifdef XY
      setXY(7,2);
      sendChar('1');sendChar('0'); // FIXME use currentID
      setXY(14,2);
      ...
      #ifdef XY2force
        sendStrXY("10",7,4); // FIXME use currentID
        if ((block%10) == 0) {
...
bool getData() {
  // Used in conjunction wite writeData()
  // Gets the next bit or byte from the current file, and sets up currentByte/currentBit/etc
  // ready for generating pulses

  bool gotNewByte = false;
  ...
  if(currentBit==0) {                         //Check for byte end/first byte
    ...
    if(bytesToRead == 0) {                  //Check for end of data block
      ...
      return gotNewByte;                               // exit
    } else if(bytesToRead == 1) {
      // only play back the bits needed
...
void writeData() {
  // Convert bits (from the currentByte) into a pulse.  One pulse per pass, two pulses per bit.
  // Shifts currentByte
  #ifdef DIRECT_RECORDING
...
void writeHeader2() {
  ...
  if(currentBit==0) {                         //Check for byte end/new byte                         
    if(hdrptr==19) {              // If we've reached end of header block send checksum byte
      currentByte = bitChecksum;
      // TODO HOW DOES THIS WORK THEN? IF BLOCKTASK IS SET TO PAUSE, HOW DOES THE CHECKSUM GET WRITTEN?
  ...
  // finally convert bits to pulses in the usual way
  writeData();
}  // End writeHeader2()
#endif
