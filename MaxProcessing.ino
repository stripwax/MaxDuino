...
bool one_buffer_not_filled_yet;

void TZXLoop() {   
  isStopped = pauseOn;
  if(BUFFER_CAN_ACCEPT(2)){                    // Keep filling until full
    TZXProcess();                           //generate the next period to add to the buffer
    if(currentPeriod) {
      BUFFER_WRITE_PUSH(currentPeriod >> 8);   //add period to the buffer
      BUFFER_WRITE_PUSH(currentPeriod & 0xff);   //add period to the buffer
    }
  } else {
    if (!pauseOn) {
...
void TZXProcess() {
  // shortcut ID15 handler to minimize overhead here
...
// used by ISR **ONLY**
static word workingPeriod;
static bool pauseFlipBit;
static unsigned long newTime;
#ifdef DIRECT_RECORDING
static word directSampleLength;
#endif
...
//  unsigned long zeroTime = micros();
  ...
  workingPeriod = word(BUFFER_READ_PEEK, BUFFER_READ_PEEK_NEXT);
  #ifdef DIRECT_RECORDING
  if ((workingPeriod & 0xc000) == 0x4000) // bit 15 clear and bit 14 set
  {
    if (workingPeriod & 0x2000) // bit 13
    {
    ...
    //      +-- this is bit 10
    //
    pinState = (workingPeriod & 0x0400); // bit 10
    if (pinState) WRITE_HIGH; else WRITE_LOW;
    ...
    if (workingPeriod & 7)
    {
      ...
      BUFFER_REWRITE((byte)(workingPeriod >> 8));
      BUFFER_REWRITE_NEXT((byte)(workingPeriod & 0x00ff));
      return;
    }
    else
    {
  ...
  if (isPauseBlock)
  {
    if(pauseFlipBit)
    {
      ...
      pinState = (bool)TSXCONTROLzxpolarityUEFSWITCHPARITY;
