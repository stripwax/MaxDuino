...
//  unsigned long zeroTime = micros();
  ...
  if (isPauseBlock)
  {
    if(pauseFlipBit)
    {
      ...
      readBuffer[readpos] = workingPeriod /256;  //reduce pause by 1ms as we've already pause for 1.5ms
      readBuffer[readpos+1] = workingPeriod  %256;  //reduce pause by 1ms as we've already pause for 1.5ms                 
      pauseFlipBit=false;
      goto _set_period;  // skips the part where we move readpos += 2 because we're using the same readpos now
