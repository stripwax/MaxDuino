...
* V4.04 Adds Arduino Nano ESP32 support, and enables Recording on SAMD21!
...
* V1.70 aka "optiboot". To flash this complete release in your Arduino 328, ask your seller to burn optiboot first !! 
...
* V1.65 Code rearrangement for easier pinout modification. Support for the new Maple miniduino (STM32) from Antonio Villena.
...
* V1.44 New define btnRoot_AS_PIVOT in userconfig.h so it can be deactivated by user. New #define in Maxduino.h to support Antonio Villena's MINIDUINO new design with amplifier and new define OLED_SETCONTRAS in userconfig.h for contrast value override. New Miniduino logo. It's possible to select RECORD_EEPROM and LOAD_EEPROM both for better testing when selecting new logos in userconfig.h, pressing MENU simulates a reset to show logo again.
...
* V1.37 Minor optimizations for counter format, acorn standard/turbo speed, simplification in polarity handling and now controlled from menu using the same option for tsx acceleration: tsxSPEEDzxPOL.
...
* V1.33 Blocks for manual REW/FF expanded from 10 up to 20, used as circular buffer. On Oled screen prints upto 99 blocks, 
...
* V1.28 Better speed loading tzx files, more than 5000 baud using PORT instructions
